...
public interface ScreenshotRepository extends JpaRepository<Screenshot, String> {
    ...
    @Query("SELECT a FROM Screenshot a WHERE a.user.id = :userId AND a.capturedAt <= :to")
    List<Screenshot> findByUserIdAndCapturedAtBefore(String userId, Date to);

    @Query("SELECT a FROM Screenshot a WHERE a.user.id = :userId AND a.capturedAt >= :from")
    List<Screenshot> findByUserIdAndCapturedAtAfter(String userId, Date from);

    @Query("SELECT a FROM Screenshot a WHERE a.user.id = :userId AND a.capturedAt >= :from AND a.capturedAt <= :to")
